...
class TestClient(unittest.TestCase):
    ...
    def test_advanced_track(self):
        ...
        self.assertEqual(msg["context"]["library"], {"name": "analytics-python", "version": VERSION})
    ...
    def test_user_defined_upload_size(self):
        client = Client("testsecret", on_error=self.fail, upload_size=10, upload_interval=3)
        ...
        # each time.
        with mock.patch("segment.analytics.consumer.post", side_effect=mock_post_fn) as mock_post:
            for _ in range(20):
                client.identify("userId", {"trait": "value"})
            time.sleep(1)
