...
class TestRequests(unittest.TestCase):
    ...
    def test_should_not_timeout(self):
        res = post("testsecret", batch=[{"userId": "userId", "event": "python event", "type": "track"}], timeout=15)
        self.assertEqual(res.status_code, 200)

    def test_should_timeout(self):
        with self.assertRaises(requests.ReadTimeout):
            post("testsecret", batch=[{"userId": "userId", "event": "python event", "type": "track"}], timeout=0.0001)
    ...
    def test_proxies(self):
        ...
        with mock.patch("segment.analytics.request._session.post", side_effect=mock_post_fn) as mock_post:
            res = post("testsecret", proxies=proxies, batch=[{"userId": "userId", "event": "python event", "type": "track"}])
            self.assertEqual(res.status_code, 200)
    ...
    def test_authorization_header_oauth(self):
        ...
        with mock.patch("segment.analytics.request._session.post", side_effect=mock_post_fn) as mock_post:
            post("testsecret", oauth_manager=oauth_manager, batch=[{"userId": "userId", "event": "python event", "type": "track"}])
    ...
    def test_x_retry_count_header(self):
        ...
        with mock.patch("segment.analytics.request._session.post", side_effect=mock_post_fn) as mock_post:
            ...
            post("testsecret", retry_count=0, batch=[{"userId": "userId", "event": "python event", "type": "track"}])
        ...
        with mock.patch("segment.analytics.request._session.post", side_effect=mock_post_fn) as mock_post:
            ...
            post("testsecret", retry_count=5, batch=[{"userId": "userId", "event": "python event", "type": "track"}])
